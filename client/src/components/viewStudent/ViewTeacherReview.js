...
const ReviewerInformation = () => {
    ...
    useEffect(() => {
        const fetchReviewAssignment = async () => {
            try {
                ...
                const response = await axios.get(`${apiUrl}/reviewAssignment/get-group-teacher-reviewers`, {
                    headers: {
    ...
    return (
        <div className="reviewer-information" style={{ width: "100%", background: "#f9f9f9", padding: "15px" }}>
            <h2>Thông Tin Giảng Viên Phản Biện</h2>
            <div className="status-info">
                <p>Ngày Phân Công: {new Date(reviewers.assignedDate).toLocaleString()}</p>
            </div>
            <div className="reviewers-list">
                {reviewers.reviewers.map((reviewer, index) => (
                    <div key={index} className="reviewer-card">
                        <h3>Giảng Viên Phản Biện {index + 1}</h3>
                        <p><strong>Tên:</strong> {reviewer.name}</p>
